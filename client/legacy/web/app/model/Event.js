    ...
    },
    {
      ...
      convert: function (v, rec) {
        ...
        if (rec.get("type") === "commandResult") {
          text =
            Strings.eventCommandResult + ": " + rec.get("attributes")["result"];
        } else if (rec.get("type") === "alarm") {
          alarmKey = rec.get("attributes")["alarm"];
          alarmKey =
            "alarm" + alarmKey.charAt(0).toUpperCase() + alarmKey.slice(1);
          text = Strings[alarmKey] || alarmKey;
        } else if (rec.get("type") === "textMessage") {
          text =
            Strings.eventTextMessage + ": " + rec.get("attributes")["message"];
        } else if (rec.get("type") === "driverChanged") {
          text =
            Strings.eventDriverChanged +
            ": " +
            Traccar.AttributeFormatter.driverUniqueIdFormatter(
        ...
        if (rec.get("maintenanceId")) {
          maintenance = Ext.getStore("Maintenances").getById(
