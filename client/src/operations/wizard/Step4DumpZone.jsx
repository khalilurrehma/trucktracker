// src/operations/wizard/Step4DumpZone.jsx
...
export default function Step4DumpZone({ goNext, goPrev }) {
  ...
  const handleNext = () => {
    ...
    if (!zone.geofence?.geometry) {
      return Swal.fire("Validation", "Please draw the dump area", "warning");
    }
    ...
    setDumpZone(zonePayload);

    Swal.fire({
      icon: "success",
      title: "DUMP_AREA",
      text: "DUMP_AREA data saved successfully.",
      timer: 1000,
  ...
  const otherGeofences = useMemo(
    ...
    [queueZone, zoneArea, loadPadZone, dumpZone]
  );
  ...
  return (
    <PageLayout
      menu={<OperationsMenu />}
      breadcrumbs={["Operations", "DUMP_AREA (Step 4/7)"]}
    >
      <Container maxWidth="xl" className={classes.container}>
        <Accordion defaultExpanded>
          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
            <Typography variant="subtitle1">
              Create DUMP_AREA for Operation: {operation?.name || "(no name)"}
            </Typography>
          </AccordionSummary>
