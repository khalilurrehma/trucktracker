...
const SocketController = () => {
  const { setTraccarUser } = useAppContext();
  ...
  const traccarUser = useSelector((state) => state.session?.user);

  useEffect(() => {
    if (traccarUser) {
      setTraccarUser(traccarUser);
    }
  }, [traccarUser]);

  const areObjectsEqualById = (obj1, obj2) => {
