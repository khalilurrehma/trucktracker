...
const DriverBehaviourReports = () => {
  ...
  useEffect(() => {
    if (mqttDriverBehaivor.length > 0) {
      fetchInitiallyFromAPI(userId);
    }
  }, [mqttDriverBehaivor]);
  ...
  const fetchInitiallyFromAPI = async (id) => {
    setLoading(true);
    try {
      const { data } = await axios.get(`${url}/driver/behaivor/status/${id}`);
      if (data.status) {
  ...
  const filteredData = initialRESTData?.filter((msg) => {
    ...
    const matchesDevice =
      ...
      msg.driverBehaivorEvent.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return (
    <PageLayout
      menu={<ReportsMenu />}
      breadcrumbs2={["reportTitle", "Driver Behaviour"]}
    >
          ...
          {filteredData.length === 0 ? (
            ...
            <TableBody>
              {filteredData
                ...
                .map((msg, index) => {
                  let intervalJSON = JSON.parse(msg.message);
                  let formattedBegin = formatUnixTimestamp(intervalJSON?.begin);
                  let formattedEnd = formatUnixTimestamp(intervalJSON?.end);
                  let formattedCreatedAt = convertISODate(msg?.createdAt);
                  ...
                  return (
                    <TableRow key={index}>
                      ...
                      <TableCell>{msg?.driverBehaivorEvent}</TableCell>
...
export default DriverBehaviourReports;
