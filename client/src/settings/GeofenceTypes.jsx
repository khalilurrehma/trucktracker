...
const GeofenceTypes = () => {
  ...
  useEffect(() => {
    const filteredData = geofencesTypes.filter((item) =>
      item.name.toLowerCase().includes(search.toLowerCase())
    );
    setFilteredItems(filteredData);
  }, [search, geofencesTypes]);
  ...
  return (
    <PageLayout
      menu={<SettingsMenu />}
      breadcrumbs={["settingsTitle", "Geofence Types"]}
    >
      <ToastContainer />
                    ...
                    <TableCell>
                      <Tooltip title={t("sharedEdit")}>
                        <IconButton
                          size="small"
                          onClick={() => {
                            navigate(`/settings/edit/geofence-type/${item.id}`);
                          }}
                        >
      ...
      <CollectionFab editPath="/settings/geofence-type" />
    </PageLayout>
  );
};
